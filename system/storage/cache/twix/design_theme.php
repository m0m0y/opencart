...
/* Twix ver. 1.18  Source: C:\xampp\htdocs\opencart\admin\view\template\design\theme.twig */
class design_theme extends Twix_Template
{
...
$('select[name="store_id"]').on('change', function(e) {
	$.ajax({
		url: 'index.php?route=design/theme/path&user_token=<?php echo @$user_token ?>&store_id=' + $('select[name="store_id"]').val(),
		dataType: 'json',
		beforeSend: function() {
...
$('#path').on('click', 'a.directory', function(e) {
	...
	$.ajax({
		url: 'index.php?route=design/theme/path&user_token=<?php echo @$user_token ?>&store_id=' + $('select[name="store_id"]').val() + '&path=' + $(node).attr('href'),
		dataType: 'json',
		beforeSend: function() {
...
$('#path').on('click', 'a.file', function(e) {
	...
	if (!$('#tab-' + tab_id).length) {
		$.ajax({
			url: 'index.php?route=design/theme/template&user_token=<?php echo @$user_token ?>&store_id=' + $('select[name="store_id"]').val() + '&path=' + $(node).attr('href'),
			dataType: 'json',
			beforeSend: function() {
				$(node).find('i').removeClass('fa-arrow-right');
				$(node).find('i').addClass('fa-circle-o-notch fa-spin');
...
$('.tab-content').on('click', '.btn-primary', function(e) {
	...
	$.ajax({
		url: 'index.php?route=design/theme/save&user_token=<?php echo @$user_token ?>&store_id=' + $('.tab-content .active input[name="store_id"]').val() + '&path=' + $('.tab-content .active input[name="path"]').val(),
		type: 'post',
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				$('#history').load('index.php?route=design/theme/history&user_token=<?php echo @$user_token ?>');
			}
		},
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
$('.tab-content').on('click', '.btn-danger', function(e) {
	if (confirm('<?php echo @$text_confirm ?>')) {
		...
		$.ajax({
			url: 'index.php?route=design/theme/reset&user_token=<?php echo @$user_token ?>&store_id=' + $('.tab-content .active input[name="store_id"]').val() + '&path=' + $('.tab-content .active input[name="path"]').val(),
			dataType: 'json',
			beforeSend: function() {
				$(node).button('loading');
			},
...
$('#history').on('click', '.btn-primary', function(e) {
	...
	if (!$('#tab-' + tab_id).length) {
		$.ajax({
			url: 'index.php?route=design/theme/template&user_token=<?php echo @$user_token ?>&store_id=' + $(node).parent().parent().find('input[name="store_id"]').val() + '&path=' + $(node).parent().parent().find('input[name="path"]').val(),
			dataType: 'json',
			beforeSend: function() {
				$(node).button('loading');
			},
