...
/* Twix ver. 1.18  Source: C:\xampp\htdocs\opencart\admin\view\template\common\filemanager.twig */
class common_filemanager extends Twix_Template
{
    ...
    {
    extract($context); ?><div id="filemanager" class="modal-dialog modal-lg">
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <?php
        ...
        $context['_seq'] = twix_ensure_traversable(twix_array_batch(@$images, 4));
        foreach ($context['_seq'] as $_key => $image) {
?>      <div class="row">
        <?php
            @$save18466 = $image            ;
            $context['_parent'] = $context;
            $context['_seq'] = twix_ensure_traversable(@$image);
            foreach ($context['_seq'] as $_key => $image) {
...
?>$('a.thumbnail').on('click', function(e) {
	...
	<?php
            if (@$thumb) {
?>	$('#<?php echo twix_escape_filter($this->env, @$thumb, "js") ?>').find('img').attr('src', $(this).find('img').attr('src'));
	<?php
            }
?>
	$('#<?php echo twix_escape_filter($this->env, @$target, "js") ?>').val($(this).parent().find('input').val());
...
$('#button-search').on('click', function(e) {
	var url = 'index.php?route=common/filemanager&user_token=<?php echo @$user_token ?>&directory=<?php echo @$directory ?>';
	...
	<?php
        if (@$thumb) {
?>	url += '&thumb=' + '<?php echo twix_escape_filter($this->env, @$thumb, "js") ?>';
	<?php
        }
?>
	<?php
        if (@$target) {
?>	url += '&target=' + '<?php echo twix_escape_filter($this->env, @$target, "js") ?>';
	<?php
        }
?>
...
$('#button-upload').on('click', function() {
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file[]\']').val() != '') {
			...
			$.ajax({
				url: 'index.php?route=common/filemanager/upload&user_token=<?php echo @$user_token ?>&directory=<?php echo @$directory ?>',
				type: 'post',
				dataType: 'json',
...
$('#button-folder').on('shown.bs.popover', function() {
	$('#button-create').on('click', function() {
		$.ajax({
			url: 'index.php?route=common/filemanager/folder&user_token=<?php echo @$user_token ?>&directory=<?php echo @$directory ?>',
			type: 'post',
			dataType: 'json',
...
$('#modal-image #button-delete').on('click', function(e) {
	if (confirm('<?php echo @$text_confirm ?>')) {
		$.ajax({
			url: 'index.php?route=common/filemanager/delete&user_token=<?php echo @$user_token ?>',
			type: 'post',
			dataType: 'json',
