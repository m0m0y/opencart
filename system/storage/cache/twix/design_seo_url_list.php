...
/* Twix ver. 1.18  Source: C:\xampp\htdocs\opencart\admin\view\template\design\seo_url_list.twig */
class design_seo_url_list extends Twix_Template
{
...
$language         = $save60655 ?>                
      ...
      <div class="col-md-9 col-md-pull-3 col-sm-12">
        ...
        if ((@$sort == "query")) {
?><a href="<?php echo @$sort_query ?>" class="<?php echo strtolower(@$order) ?>"><?php echo @$column_query ?></a><?php
        } else {
?><a href="<?php echo @$sort_query ?>"><?php echo @$column_query ?></a><?php
        }
?></td>
                      <td class="text-left"><?php
        if ((@$sort == "keyword")) {
?><a href="<?php echo @$sort_keyword ?>" class="<?php echo strtolower(@$order) ?>"><?php echo @$column_keyword ?></a> <?php
        } else {
?><a href="<?php echo @$sort_keyword ?>"><?php echo @$column_keyword ?></a><?php
        }
?></td>
                      <td class="text-left"><?php
        if ((@$sort == "store")) {
...
?>                  <?php
            @$save26288 = $seo_url            ;
            $context['_parent'] = $context;
            $context['_seq'] = twix_ensure_traversable(@$seo_urls);
            foreach ($context['_seq'] as $_key => $seo_url) {
?>                  <tr>
                    <td class="text-center"><?php
                if (twix_in_filter(@$seo_url["seo_url_id"], @$selected)) {
...
$('#button-filter').on('click', function() {
	var url = 'index.php?route=design/seo_url&user_token=<?php echo @$user_token ?>';
