...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$total_s = $this->model_checkout_order->getMytotalAmount($order_info['order_id']);
		
		$url = "";
		if($order_info['payment_method']=="Dragonpay"){
			...
			// $merchantpass = "4icGoPFWgta3pgV";
			// $merchantid = "DL49EETHEPRESE";
			$merchantpass = "Fk3MvXTy";
			$merchantid = "INMED";
			$txnid = $order_info['order_id'];
			$amount = (float)$total_s;
			$ccy = "PHP";
			$description = "TMS Products";
		...
		foreach ($order_products as $order_product) {
			...
			$data['products'][] = array(
				'image'     => "https://medicalshop.ph/image/".$order_product['image'],
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			$data['order_image'] = $order_info['image'];
