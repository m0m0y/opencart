...
class ModelExtensionTotalReferral extends Model {
    public function getReferral($code) {
        ...
        if($referral_query->num_rows) {
            ...
            $referral_category_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon_category` cc LEFT JOIN `" . DB_PREFIX . "category_path` cp ON (cc.category_id = cp.path_id) WHERE cc.coupon_id = '" . (int)$referral_query->row['coupon_id'] . "'");
            ...
            if($referral_product_data || $referral_category_data) {
    ...
    public function confirm($order_info, $order_total) {
		...
		if ($code) {
			...
			$referral_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code) . "' AND status = '1'");
			...
			if ($status) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "coupon_history` SET coupon_id = '" . (int)$referral_query->row['coupon_id'] . "', order_id = '" . (int)$order_info['order_id'] . "', customer_id = '" . (int)$order_info['customer_id'] . "', amount = '" . (float)$order_total['value'] . "', date_added = NOW()");
			} else {
